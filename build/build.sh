...
WHISPER_LIBRARY_PATH="/tmp/whisper.cpp-${WHISPER_VERSION}/build/src"
...
NEMO_INCLUDE_PATH="/opt/nemo-speech/include"
NEMO_LIBRARY_PATH="/opt/nemo-speech/lib"
...
INCLUDE_PATH="${OPUS_INCLUDE_PATH}:${ONNX_INCLUDE_PATH}:${AZURE_SDK_INCLUDE_PATH}:${NEMO_INCLUDE_PATH}"
LIB_PATH="${OPUS_LIBRARY_PATH}:${ONNX_LIBRARY_PATH}:${AZURE_SDK_LIBRARY_PATH}:${NEMO_LIBRARY_PATH}"
if [ "${NEMOSPEECH:-0}" != "1" ]; then
	INCLUDE_PATH="${WHISPER_INCLUDE_PATH}:${INCLUDE_PATH}"
	LIB_PATH="${WHISPER_LIBRARY_PATH}:${LIB_PATH}"
fi
